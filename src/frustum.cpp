...
{
	if (!frustum.is_ortho) {
		return perspective_matrix(frustum.aspect_x, frustum.aspect_y,
					  frustum.shift_x, frustum.shift_y,
					  frustum.near, frustum.far);
	} else {
		return orthographic_matrix(frustum.aspect_x, frustum.aspect_y,
					   frustum.shift_x, frustum.shift_y,
					   frustum.near, frustum.far);
	}
}
...
Mat4 projection_matrix_inv(const CameraFrustum &frustum)
{
	if (!frustum.is_ortho) {
		return perspective_matrix_inv(
		    frustum.aspect_x, frustum.aspect_y, frustum.shift_x,
		    frustum.shift_y, frustum.near, frustum.far);
	} else {
...
Mat4 perspective_matrix(float ax, float ay, float sx, float sy, float n,
			float f)
{
	assert(n != f);
...
Mat4 perspective_matrix_inv(float ax, float ay, float sx, float sy, float n,
			    float f)
{
...
Mat4 orthographic_matrix(float ax, float ay, float sx, float sy, float n,
			 float f)
{
	assert(n != f);
...
Mat4 orthographic_matrix_inv(float ax, float ay, float sx, float sy, float n,
			     float f)
{
	assert(n != f);
